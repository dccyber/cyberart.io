...
class MandelbrotAnimation extends StateMachineAnimation {
  ...
  stateTransition(i, j, width, height, state, nextState) {
    ...
    if (!(cellState.escaped || cellState.dead)) {
      ...
      let newReal =
        (cellState.real + cellState.imag) * (cellState.real - cellState.imag);
  ...
  colorGenerator(cellState, framesElapsed) {
    ...
    if (cellState.escaped) {
      ...
      return {
        red:
          (cellState.iter * Math.abs(Math.sin(framesElapsed / 101)) * 89) % 256,
        green:
          (cellState.iter * Math.abs(Math.sin(framesElapsed / 61)) * 67) % 256,
        blue:
          (cellState.iter * Math.abs(Math.sin(framesElapsed / 107)) * 20) % 256,
